...
#define MPU6050_AXOFFSET 310
#define MPU6050_AYOFFSET 62
#define MPU6050_AZOFFSET 1460
...
#define MPU6050_GYOFFSET -22
#define MPU6050_GZOFFSET -62
...
#define IN3 10   // Direction B
#define IN4 7   // Direction B

double motorSpeedFactorLeft = 1;
double motorSpeedFactorRight = 0.8;
int MIN_ABS_SPEED = 140;
LMotorController motorController(ENA, IN1, IN2, ENB, IN4, IN3, motorSpeedFactorLeft, motorSpeedFactorRight);
int pwm = 0;
//---------------------------------------------------------------------------------------------------

//A observador
double a11 = 0.0004,      a12 = 0.0001,     a13 = 0,   a14 = 0;
double a21 = -0.0874,     a22 = -49.8953,   a23 = 0,   a24 = 0;
double a31 = -111.7091,   a32 = 0,          a33 = 0,   a34 = 0.0001;
double a41 = -336.5982,   a42 = 0,          a43 = 0,   a44 = 0;

//B observador
double b11 = 0,     b12 = -0.0004;
double b21 = 0.005, b22 = 0.0972;
double b31 = 0,     b32 = 111.7091;
double b41 = 0,     b42 = 336.5982;
...
double k1 = 7.2437, k2 = -0.5056, k3 = -70.4188, k4 = -99.6305;
float Ts = 0.01; 
...
void loop() {
  ...
  x1_hat = Ts*(a11*x1_hat + a12*x1d_hat + a13*x2_hat + a14*x2_hat + b11*u + b12*pitch);
  x1d_hat = Ts*(a21*x1_hat + a22*x1d_hat + a23*x2_hat + a24*x2_hat + b21*u + b22*pitch);
  x2_hat =  Ts*(a31*x1_hat + a32*x1d_hat + a33*x2_hat + a34*x2_hat + b31*u + b32*pitch);
  x2d_hat =  Ts*(a41*x1_hat + a42*x1d_hat + a43*x2_hat + a44*x2_hat + b41*u + b42*pitch);


  // control signal
  u = -(k1*x1_hat + k2*x1d_hat + k3*x2_hat + k4*x2d_hat);
  umax = 30000;
  umin = -30000;

  //Serial.println(u);
  pwm = map(u,umin,umax,-255,255);

  motorController.move(pwm, MIN_ABS_SPEED);

  // Print Roll, Pitch, Yaw to Serial Monitor
  //Serial.print(pitch);Serial.print(";");Serial.print(pwm);
  //Serial.println();
}
